...
defaults in `<plugin-root>/standards/`. This skill captures them once
into `.conventions.md` at the repo root, so every other skill can honor them.

## 0. Resolve the repo

Resolve `<work-repo>` per `<plugin-root>/core/repo-resolution.md`: the
...
- Every convention cites at least one real example (`file:line`). No example →
  ...
  `<plugin-root>/core/decision-protocol.md` §1 applied to profiling:
...
`<plugin-root>/templates/domain-glossary.md`) with the repo's obvious
