...
2. Resolve `<work-repo>`, `<workspace-root>`, and the `<repo-set>` per
   `<plugin-root>/core/repo-resolution.md` (cwd is not assumed to be
   the repo; sibling repos are in analysis scope by default), then create the
   workspace:
   `<plugin-root>/scripts/new-task.sh <task-id> <workspace-root> change-request <work-repo> <repo-set>`
   (or resume from an existing STATUS.md, taking all of them from it).
3. Store the CR text as `work/<id>/brief.md`.
4. Unknowns follow `<plugin-root>/core/decision-protocol.md`:
...
2. Find ALL callers and dependents **in every repo of `<repo-set>`** (from
   ...
   `<plugin-root>/scripts/list-repos.sh <repos-folder>` and record the
...
1. Self-check `<plugin-root>/checklists/change-request.md`. For
   multi-file changes, also run the audit procedure in
   `<plugin-root>/agents/scope-auditor.md` against the mini contract
