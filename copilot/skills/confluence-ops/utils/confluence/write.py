...
USAGE:
    ...
    python3 utils/confluence/write.py move PAGE_ID --space TARGET_SPACE_KEY --yes [--parent PARENT_ID]

    python3 utils/confluence/write.py search "CQL query" [--limit 25]

    python3 utils/confluence/write.py comments PAGE_ID [--limit 50]

    python3 utils/confluence/write.py notifications [--days 1] [--spaces SPACE1,SPACE2]

ENVIRONMENT:
    JIRA_URL          - Atlassian site, e.g. https://your-org.atlassian.net
    CONFLUENCE_URL    - override when Confluence is not at <JIRA_URL>/wiki
    JIRA_EMAIL        - Atlassian email
    JIRA_TOKEN        - Atlassian API token (works for Confluence too)
    CONFLUENCE_SPACES - default space keys for `notifications`
...
NOTES:
    ...
    - For create, --space is required (a space key, or ~<accountId> for a personal space)
    - For move, the page is COPIED to the target then the original deleted (new page
      ID; history/comments/attachments/children are not carried over) — needs --yes
...
def resolve_base_url():
    """Confluence base URL from the environment — never a hardcoded tenant.

    CONFLUENCE_URL wins. Otherwise derive it from JIRA_URL, since Jira and
    Confluence Cloud share one Atlassian site (`<site>/wiki`).
    """
    explicit = os.environ.get("CONFLUENCE_URL", "").rstrip("/")
    if explicit:
        return explicit
    jira = os.environ.get("JIRA_URL", "").rstrip("/")
    return f"{jira}/wiki" if jira else ""


CONFLUENCE_URL = resolve_base_url()

ENV_HELP = """Set these in your shell before using the confluence-ops skill:
  export JIRA_URL=https://your-org.atlassian.net   # or CONFLUENCE_URL for a separate site
  export JIRA_EMAIL=you@your-org.example
  export JIRA_TOKEN=...   # https://id.atlassian.com/manage-profile/security/api-tokens
Optional: CONFLUENCE_SPACES (default spaces for `notifications`)"""


def page_url(page_id):
    """Browser URL for a page id, on whichever site is configured."""
...
def get_session():
    """Create authenticated Confluence session. Exits on missing configuration."""
    email = os.environ.get("JIRA_EMAIL")
    token = os.environ.get("JIRA_TOKEN")
    missing = [name for name, value in (
        ("CONFLUENCE_URL or JIRA_URL", CONFLUENCE_URL),
        ("JIRA_EMAIL", email),
        ("JIRA_TOKEN", token),
    ) if not value]
    if missing:
...
def _child_counts(session, page_id):
    """(child pages, attachments) on a page — what a cross-space move leaves behind."""
    counts = []
    for kind in ("page", "attachment"):
        r = session.get(f"{CONFLUENCE_URL}/rest/api/content/{page_id}/child/{kind}",
                        params={"limit": 1})
        counts.append(r.json().get("size", 0) if r.status_code == 200 else None)
    return tuple(counts)


def cmd_move(args):
    """Copy a page into another space, then delete the original.

    Confluence's REST API cannot move a page across spaces, so this is a
    copy-then-delete: the new page gets a NEW id, and version history,
    comments, attachments, labels, child pages and inbound links do NOT come
    with it. The copy is created and verified BEFORE the original is deleted,
    so a failure anywhere leaves the source page intact.
    """
    s = get_session()
    ...
    children, attachments = _child_counts(s, args.page_id)
    print(f"Move '{title}' ({args.page_id}): {src_space} → {args.space}")
    print("  This is a COPY then DELETE — the page gets a new id and loses:")
    print(f"    version history ({current['version']['number']} versions), comments, labels, inbound links")
    print(f"    attachments: {'unknown' if attachments is None else attachments}")
    print(f"    child pages: {'unknown' if children is None else children}"
          f"{' (they are NOT moved and will be orphaned)' if children else ''}")
    if not args.yes:
        print("❌ Refusing to move without --yes. Re-run with --yes to confirm.", file=sys.stderr)
        sys.exit(1)

    # Step 1: create the copy in the target space FIRST, so a failure here
    ...
    if r.status_code not in (200, 201):
        ...
        print(r.text[:500], file=sys.stderr)
        print(f"   Source page {args.page_id} in {src_space} is untouched.", file=sys.stderr)
        sys.exit(1)
    new_id = r.json()["id"]

    # Step 2: verify the copy is readable and its body round-tripped before
    # deleting anything.
    v = s.get(f"{CONFLUENCE_URL}/rest/api/content/{new_id}", params={"expand": "body.storage"})
    if v.status_code != 200 or not v.json().get("body", {}).get("storage", {}).get("value"):
        print(f"❌ Created page {new_id} did not verify — source NOT deleted.", file=sys.stderr)
        print(f"   Inspect {page_url(new_id)} and delete it manually if it is bad.", file=sys.stderr)
        sys.exit(1)

    # Step 3: only now remove the original.
    r = s.delete(f"{CONFLUENCE_URL}/rest/api/content/{args.page_id}")
    if r.status_code != 204:
        print(f"⚠️  Copy created at {new_id}, but deleting the original failed: {r.status_code}",
              file=sys.stderr)
        print(r.text[:500], file=sys.stderr)
        print(f"   Both pages now exist — delete {args.page_id} in {src_space} manually.",
              file=sys.stderr)
        sys.exit(1)
...
def cmd_notifications(args):
    ...
    s = get_session()
    days = args.days
    spaces = [s for s in (args.spaces or os.environ.get("CONFLUENCE_SPACES", "")).split(",") if s]
...
def main():
    ...
    p_create.add_argument("--space", required=True, help="Space key (or ~<accountId> for a personal space)")
    ...
    p_move.add_argument("--yes", action="store_true",
                        help="Confirm the copy-then-delete; required (the page gets a new id "
                             "and loses history, comments, attachments and children)")
    ...
    p_notif.add_argument("--spaces", default=None, help="Comma-separated space keys (default: $CONFLUENCE_SPACES)")
