...
1. Load lessons tagged `stage:estate-profile` (and any tagged `platform` or
   `integration`) from `<plugin-root>/knowledge/lessons.md`.
2. Resolve `<repo-set>` and `<estate-root>` per
   `<plugin-root>/core/repo-resolution.md`. The scan covers EVERY repo
   ...
   false `absent`.
3. Unknowns follow `<plugin-root>/core/decision-protocol.md`. Note the
...
1. Enumerate repos with
   `<plugin-root>/scripts/list-repos.sh <repos-folder>` (the
...
1. Run
   `<plugin-root>/scripts/scan-integrations.sh <repo> [<repo>…]`
...
1. Copy `<plugin-root>/templates/platform-capabilities.md` →
...
1. Self-check with `<plugin-root>/checklists/estate-profile.md`.
