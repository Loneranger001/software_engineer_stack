...
1. Read `<plugin-root>/knowledge/lessons.md` and note lessons tagged
   `stage:intake` or matching this repo/language mix. Apply them.
2. Resolve `<work-repo>`, `<workspace-root>`, and the `<repo-set>` per
   `<plugin-root>/core/repo-resolution.md` — do NOT assume the current
   ...
   the rest with `<plugin-root>/scripts/list-repos.sh <repos-folder>`:
...
3. Unknowns follow `<plugin-root>/core/decision-protocol.md`: check
...
2. Run `<plugin-root>/scripts/new-task.sh <task-id> <workspace-root> full <work-repo> <repo-set>`
...
python3 <plugin-root>/skills/jira-ops/utils/jira/query.py issue PROJ-1234 --full

# Confluence page (id, full URL, or /wiki/x/ short link) → markdown
python3 <plugin-root>/skills/confluence-ops/utils/confluence/pull_confluence_page.py <page-id-or-url> work/<id>/brief.md
```
...
1. Copy `<plugin-root>/templates/scope-contract.md` to
...
3. Run `<plugin-root>/checklists/intake.md` against your draft; fix gaps.
