...
4. Unknowns follow `<plugin-root>/core/decision-protocol.md`: design
...
5. Prose follows `<plugin-root>/core/writing-style.md` — active voice
   with a named actor, one idea per sentence, one name per object.

## 1. Design

1. Copy `<plugin-root>/templates/tdd.md` → `work/<id>/tdd.md`.
...
4. Design each component against the repo's conventions (`.conventions.md`,
   else `<plugin-root>/standards/<language>.md`): naming, error
...
1. Self-check with `<plugin-root>/checklists/tech-design.md`.
2. Run the fact-check procedure in
   `<plugin-root>/agents/doc-fact-checker.md` on tdd.md (it verifies
