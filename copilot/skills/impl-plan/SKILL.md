...
1. Copy `<plugin-root>/templates/impl-doc.md` → `work/<id>/impl-doc.md`.
...
4. Name a task branch **per changed repo** (each following its own repo's
   ...
   `<plugin-root>/core/repo-resolution.md` — e.g. `feature/<task-id>`).
...
1. Self-check with `<plugin-root>/checklists/impl-plan.md`.
