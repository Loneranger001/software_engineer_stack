...
python3 ${CLAUDE_PLUGIN_ROOT}/skills/jira-ops/utils/jira/query.py issue PROJ-1234 --full

# Confluence page (id, full URL, or /wiki/x/ short link) → markdown
python3 ${CLAUDE_PLUGIN_ROOT}/skills/confluence-ops/utils/confluence/pull_confluence_page.py <page-id-or-url> work/<id>/brief.md
```
...
1. Present the scope contract to the user: objective, in-scope list, exclusions,
   acceptance criteria, open questions — plus any open ASSUMPTIONS.md entries
   for ratification (decision-protocol §4). Ask explicitly: "Approve this scope
