...
Resolve `<work-repo>` per `${CLAUDE_PLUGIN_ROOT}/core/repo-resolution.md`: the
...
- Every convention cites at least one real example (`file:line`). No example →
  it goes under Unresolved, not asserted. (This is
  `${CLAUDE_PLUGIN_ROOT}/core/decision-protocol.md` §1 applied to profiling:
  conventions are checkable facts, never estimated.)
...
`${CLAUDE_PLUGIN_ROOT}/templates/domain-glossary.md`) with the repo's obvious
