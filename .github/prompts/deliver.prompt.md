...
2. Locate the workspace; verify the prior stage is done (`implement: done`,
   ...
   (`./core/repo-resolution.md`) — handover paths and git
...
3. Unknowns follow `./core/decision-protocol.md`; at this
...
1. Resolve ASSUMPTIONS.md first: every `open` entry is presented to the user
   ...
   an unratified guess.
2. Run the audit procedure in `./agents/scope-auditor.md`
...
1. Fill `./templates/release-notes.md` →
...
```bash
# first publish
python3 ./skills/confluence-ops/utils/confluence/write.py \
...
```bash
python3 ./skills/jira-ops/utils/jira/write.py \
...
1. Self-check with `./checklists/deliver.md`.
