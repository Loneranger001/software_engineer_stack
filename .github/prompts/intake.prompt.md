...
> Arguments: `<path-to-brief-or-pasted-requirements> [task-id]`
...
2. Resolve `<work-repo>`, `<workspace-root>`, and the `<repo-set>` per
   ...
   the rest with `./scripts/list-repos.sh <repos-folder>`:
...
3. Unknowns follow `./core/decision-protocol.md`: check
...
2. Run `./scripts/new-task.sh <task-id> <workspace-root> full <work-repo> <repo-set>`
...
1. Copy `./templates/scope-contract.md` to
...
3. Run `./checklists/intake.md` against your draft; fix gaps.
